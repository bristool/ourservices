...
if ( ! function_exists( 'ogma_news_show_widget_field' ) ) :
     ...
     * @since 1.0.0
     */
    function ogma_news_show_widget_field( $instance = '', $widget_field = '', $widget_field_value = '' ) {
        ...
        switch ( $widget_field_type ) {
            ...
            ?>
                    <p class="post-cat ogma-news-widget-field-wrapper <?php echo esc_attr( $widget_field_wrapper ); ?>">
                        <label for="<?php echo esc_attr( $instance->get_field_id( $widget_field_name ) ); ?>"><?php echo esc_html( $widget_field_title ); ?>:</label>
                        <?php if ( isset( $widget_field_description ) ) { ?>
                           <span class="field-description"><em><?php echo wp_kses_post( $widget_field_description ); ?></em></span>
                        <?php }
                            ...
                            $dropdown_args = wp_parse_args( array(
                                'taxonomy'          => 'category',
                                'show_option_none'  => __( '- - Select Category - -', 'ogma-news' ),
            ...
            ?>
                <div class="attachment-media-view">
                    ...
                    <div class="placeholder<?php echo esc_attr( $image_class ); ?>">
                        <?php esc_html_e( 'No image selected', 'ogma-news' ); ?>
                    </div>
                    <div class="thumbnail thumbnail-image">
                        <?php echo $image; ?>
                    </div>

                    <div class="actions ogma-news-clearfix">
                        <button type="button" class="button ogma-news-delete-button align-left"><?php esc_html_e( 'Remove', 'ogma-news' ); ?></button>
                        <button type="button" class="button ogma-news-upload-button alignright"><?php esc_html_e( 'Select Image', 'ogma-news' ); ?></button>
...
if ( ! function_exists( 'ogma_news_widget_updated_field_value' ) ) :
     ...
     *
     */
    function ogma_news_widget_updated_field_value( $widget_field, $new_field_value ) {
...
if ( ! function_exists( 'ogma_news_trending_posts_widget_layout_choices' ) ) :
     ...
     * @since 1.0.0
     */
    function ogma_news_trending_posts_widget_layout_choices() {
        $widget_layout = apply_filters( 'ogma_news_trending_posts_widget_layout_choices',
            array(
                'trending-layout--one'  => array(
                    'label'     => esc_html__( 'Layout One', 'ogma-news' ),
                    'img_path'  => get_template_directory_uri() . '/inc/widgets/assets/images/latest-post-widget-layout-one.png'
                ),
                'trending-layout--one'  => array(
                    'label'     => esc_html__( 'Layout Two', 'ogma-news' ),
...
if ( ! function_exists( 'ogma_news_posts_query_filter_choices' ) ) :
     ...
     * @since 1.0.0
     */
    function ogma_news_posts_query_filter_choices() {
        $post_query_filter = apply_filters( 'ogma_news_posts_query_filter_choices',
            array(
                'latest'  => __( 'Latest Posts', 'ogma-news' ),
                'random'  => __( 'Random Posts', 'ogma-news' )
            )
        );
...
if ( ! function_exists( 'ogma_news_trending_posts_filter_choices' ) ) :
     ...
     * @since 1.0.0
     */
    function ogma_news_trending_posts_filter_choices() {
        $trending_posts_filter = apply_filters( 'ogma_news_trending_posts_filter_choices',
            array(
                'tag'      => __( 'Tag', 'ogma-news' ),
                'comment'  => __( 'Comment', 'ogma-news' )
            )
        );
