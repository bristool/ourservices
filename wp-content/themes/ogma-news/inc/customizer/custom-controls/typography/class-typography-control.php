...
if ( ! class_exists( 'Ogma_News_Control_Typography' ) ) {
     ...
     * @since 1.0.0
     */
    class Ogma_News_Control_Typography extends WP_Customize_Control {
         ...
         * @var    string
         */
        public $type = 'ogma-news-typography';
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->l10n = wp_parse_args(
                $this->l10n,
                array(
                    'family'        => esc_html__( 'Font Family', 'ogma-news' ),
                    'weight'        => esc_html__( 'Font Weight', 'ogma-news' ),
                    'style'         => esc_html__( 'Font Style', 'ogma-news' ),
                    'transform'     => esc_html__( 'Font Transform', 'ogma-news' ),
                    'decoration'    => esc_html__( 'Font Decoration', 'ogma-news' )
                )
            );
        }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script( 'ogma-news-typo-ajax-script', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/typo-ajax.js', array( 'jquery', 'select2' ), false, true );
            
            wp_localize_script( 'ogma-news-typo-ajax-script', 'ajax_script', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
        }
        ...
        function get_font_families() {
            ...
            $ogma_news_google_font = get_option( 'ogma_news_google_font' );

            foreach ( $ogma_news_google_font as $key => $value ) {
        ...
        public function get_font_weight_choices() {
            ...
            if ( $this->settings['family']->id ) {
                ...
                $get_font_family = ogma_news_get_customizer_option_value( $font_family_id );
                $variants_array = $ogma_news_font_list[$get_font_family]['0'];

                if ( is_array( $variants_array ) ) {
                    $options_array = array(
                        'inherit' => __( 'Inherit', 'ogma-news' )
                    );
                    foreach ( $variants_array  as $variants ) {
                        $options_array[$variants] = ogma_news_convert_font_variants( $variants );
                    }

                    return $options_array;
                } else {
                    return array(
                        'inherit'   => esc_html__( 'Inherit', 'ogma-news' ),
                        '400'       => esc_html__( 'Normal 400', 'ogma-news' ),
                        '700'       => esc_html__( 'Bold 700', 'ogma-news' ),
                    );
                }
            } else {
                return array(
        ...
        public function get_font_style_choices() {
            ...
            $font_style_choices = apply_filters( 'ogma_news_font_style_choices',
                array(
                    'inherit'   => __( 'Inherit', 'ogma-news' ),
                    'normal'    => __( 'Normal', 'ogma-news' ),
                    'italic'    => __( 'Italic', 'ogma-news' ),
                )
            );
        ...
        public function get_font_transform_choices() {
            ...
            $font_transform_choices = apply_filters( 'ogma_news_font_transform_choices',
                array(
                    'inherit'       => __( 'Inherit', 'ogma-news' ),
                    'lowercase'     => __( 'Lowercase', 'ogma-news' ),
                    'capitalize'    => __( 'Capitalize', 'ogma-news' ),
                    'uppercase'     => __( 'Uppercase', 'ogma-news' ),
                )
            );
        ...
        public function get_font_decoration_choices() {
            ...
            $font_decoration_choices = apply_filters( 'ogma_news_font_decoration_choices',
                array(
                    'inherit'       => __( 'Inherit', 'ogma-news' ),
                    'underline'     => __( 'Underline', 'ogma-news' ),
                    'line-through'  => __( 'Line-through', 'ogma-news' ),
                )
            );
